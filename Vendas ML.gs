function importarUltimoExcelDaPasta(props, runId, TTL_MS) {
  ...
  if ([idxVenda, idxEstado, idxUnidades, idxTitulo, idxComprador, idxEntrega].some(i => i === -1)) {
  ...
  if (totalLinhasAdicionadas > 0) {
    const rangeDestino = abaDestino.getRange(inicioInsercao, 1, totalLinhasAdicionadas, 8);
    ...
    const valores = abaDestino.getRange(inicioInsercao, 1, totalLinhasAdicionadas, 8).getValues();
    let linhaOffset = 0;
